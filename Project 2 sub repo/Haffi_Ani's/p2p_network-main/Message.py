...
class Message:
    ...
    def short_msg(self,title,p_name="",hospital="some hospital"):

        ts=time.time()
        id=hashlib.sha256(f"{random.randint(0,99999)}{title}{p_name}{time}".encode()).hexdigest()
        msg_obj={
            "id"    :id,
            "title":title,
            "p_name":p_name,
