...
class Network(Node):
    ...
    CONN_PORT    =None
    CONN_ADDR    =None
    def __init__(self,ip="",port=None,hospital =""):
        self.SERVER_IP=ip
        ...
        while (connected):
            ...
            if (msg_len):
                ...
                if(commands.NODE_CON_ADDR in msg['title']):                                           #CONNECT TO ME COMMAND
                    ...
                    conn_addr=msg["p_name"].split(",")                                            #split ip and port
                ...
                if("#BROADCAST" == msg["title"]):
                    ...
                    if(msg['p_name']=="#JOINED_IN_NETWORK"):
    ...
    def short_json_msg(self,title,message=""):
        ...
        message=Message().short_msg(title,message,self.HOSPITAL)
        return message
    ...
    def broadcast(self,data,isJson=False):
        ...
        for node in self.nodes:
            ...
            try:
                ...
                print_colored(f'Node {node} REMOVED FROM self.nodes',"red")
